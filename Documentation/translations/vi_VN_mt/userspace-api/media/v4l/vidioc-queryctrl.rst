...
:Original: Documentation/userspace-api/media/v4l/vidioc-queryctrl.rst
...
**********************************************************************
ioctls VIDIOC_QUERYCTRL, VIDIOC_QUERY_EXT_CTRL và VIDIOC_QUERYMENU
*******************************************************************

Tên
====

VIDIOC_QUERYCTRL - VIDIOC_QUERY_EXT_CTRL - VIDIOC_QUERYMENU - Liệt kê các điều khiển và các mục điều khiển menu
...
ZZ0003ZZ
    Con trỏ tới cấu trúc ZZ0000ZZ, ZZ0001ZZ
    hoặc ZZ0002ZZ (tùy thuộc vào ioctl).

Sự miêu tả
===========

Để truy vấn các thuộc tính của ứng dụng điều khiển, hãy đặt trường ZZ0001ZZ
của cấu trúc ZZ0000ZZ và gọi
ZZ0002ZZ ioctl với một con trỏ tới cấu trúc này. Người lái xe
lấp đầy phần còn lại của cấu trúc hoặc trả về mã lỗi ZZ0003ZZ khi
ZZ0004ZZ không hợp lệ.

Có thể liệt kê các điều khiển bằng cách gọi ZZ0000ZZ
với các giá trị ZZ0001ZZ liên tiếp bắt đầu từ ZZ0002ZZ cho đến và
ZZ0003ZZ độc quyền. Trình điều khiển có thể trả về ZZ0004ZZ nếu có điều khiển trong
phạm vi này không được hỗ trợ. Các ứng dụng khác có thể liệt kê riêng tư
các điều khiển không được xác định trong thông số kỹ thuật này bằng cách bắt đầu từ
ZZ0005ZZ và tăng dần ZZ0006ZZ cho đến khi có driver
trả về ZZ0007ZZ.

Trong cả hai trường hợp, khi trình điều khiển đặt cờ ZZ0000ZZ
trong trường ZZ0001ZZ, điều khiển này bị vô hiệu hóa vĩnh viễn và sẽ
bị ứng dụng bỏ qua. [#f1]_

Khi ứng dụng HOẶC ZZ0000ZZ với ZZ0001ZZ,
trình điều khiển trả về điều khiển không kết hợp được hỗ trợ tiếp theo hoặc ZZ0002ZZ nếu
không có. Ngoài ra, cờ ZZ0003ZZ
có thể được chỉ định để liệt kê tất cả các điều khiển phức hợp (tức là các điều khiển có
nói cách khác là loại ≥ ZZ0004ZZ và/hoặc điều khiển mảng
điều khiển có chứa nhiều hơn một giá trị). Chỉ định cả hai
ZZ0005ZZ và ZZ0006ZZ trong
để liệt kê tất cả các điều khiển, phức tạp hay không. Trình điều khiển không
hỗ trợ những lá cờ này nhưng luôn trả về ZZ0007ZZ.

ZZ0001ZZ ioctl được giới thiệu để cải thiện
hỗ trợ các điều khiển có thể sử dụng các loại kết hợp và để hiển thị các điều khiển bổ sung
thông tin kiểm soát không thể được trả về trong struct
ZZ0000ZZ vì cấu trúc đó đã đầy.

ZZ0000ZZ được sử dụng theo cách tương tự như
ZZ0001ZZ, ngoại trừ mảng ZZ0002ZZ phải bằng 0
cũng vậy.

Thông tin bổ sung cần thiết cho việc điều khiển menu: tên của
các mục thực đơn. Để truy vấn các ứng dụng này, hãy đặt ZZ0002ZZ và ZZ0003ZZ
các trường của cấu trúc ZZ0000ZZ và gọi
ZZ0004ZZ ioctl với một con trỏ tới cấu trúc này. Người lái xe
lấp đầy phần còn lại của cấu trúc hoặc trả về mã lỗi ZZ0005ZZ khi
ZZ0006ZZ hoặc ZZ0007ZZ không hợp lệ. Các mục menu được liệt kê bằng cách gọi
ZZ0008ZZ với các giá trị ZZ0009ZZ liên tiếp từ cấu trúc
ZZ0001ZZ ZZ0010ZZ đến ZZ0011ZZ,
bao gồm.
...
Xem thêm các ví dụ trong ZZ0000ZZ.
...
EINVAL
    Cấu trúc ZZ0000ZZ ZZ0002ZZ là
    không hợp lệ. Cấu trúc ZZ0001ZZ ZZ0003ZZ
    không hợp lệ hoặc ZZ0004ZZ nằm ngoài phạm vi (nhỏ hơn ZZ0005ZZ hoặc
    lớn hơn ZZ0006ZZ) hoặc mục menu cụ thể này không
    được tài xế hỗ trợ.

EACCES
    Một nỗ lực đã được thực hiện để đọc điều khiển chỉ ghi.
