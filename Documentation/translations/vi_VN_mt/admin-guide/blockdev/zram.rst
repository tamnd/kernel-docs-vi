...
:Original: Documentation/admin-guide/blockdev/zram.rst
...
============================================
zram: Thiết bị khối dựa trên RAM được nén
============================================

Giới thiệu
============

Mô-đun zram tạo các thiết bị khối dựa trên RAM có tên /dev/zram<id>
(<id> = 0, 1, ...). Các trang được ghi vào các đĩa này được nén và lưu trữ
trong chính bộ nhớ của mình. Những đĩa này cho phép I/O rất nhanh và cung cấp khả năng nén
lượng tiết kiệm bộ nhớ tốt. Một số trường hợp sử dụng bao gồm lưu trữ /tmp,
sử dụng làm đĩa trao đổi, các bộ đệm khác nhau trong /var và có thể nhiều hơn nữa. :)

Thống kê cho các thiết bị zram riêng lẻ được xuất qua các nút sysfs tại
/sys/block/zram<id>/
...
a) sử dụng thuộc tính sysfs zram và zram_control
b) sử dụng tiện ích zramctl, được cung cấp bởi util-linux (util-linux@vger.kernel.org).

Trong tài liệu này, chúng tôi sẽ chỉ mô tả các bước cấu hình zram 'thủ công',
Thuộc tính sysfs IOW, zram và zram_control.

Để hiểu rõ hơn về zramctl, vui lòng tham khảo util-linux
tài liệu, trang man zramctl hoặc ZZ0000ZZ. Xin được thông báo
rằng những người bảo trì zram không phát triển/duy trì util-linux hoặc zramctl, nên
bạn có thắc mắc vui lòng liên hệ util-linux@vger.kernel.org

Sau đây trình bày trình tự các bước điển hình để sử dụng zram.

WARNING
=======

Để đơn giản, chúng tôi bỏ qua phần kiểm tra lỗi trong hầu hết các
ví dụ dưới đây. Tuy nhiên, trách nhiệm duy nhất của bạn là xử lý lỗi.

Thuộc tính zram sysfs luôn trả về giá trị âm trong trường hợp có lỗi.
Danh sách các mã trả lại có thể:

===========================================================================
-EBUSY cố gắng sửa đổi một thuộc tính không thể thay đổi một lần
	  thiết bị đã được khởi tạo. Vui lòng đặt lại thiết bị trước.
-ENOMEM zram không thể phân bổ đủ bộ nhớ để đáp ứng nhu cầu của bạn
	  nhu cầu.
-EINVAL đầu vào không hợp lệ đã được cung cấp.
-EAGAIN thử lại thao tác sau (ví dụ: khi cố chạy giải nén
	  và viết lại đồng thời).
===========================================================================

Nếu bạn sử dụng 'echo', giá trị trả về được thiết lập bởi tiện ích 'echo',
và, trong trường hợp chung, đại loại như::
...
Điều này tạo ra 4 thiết bị: /dev/zram{0,1,2,3}

Tham số num_devices là tùy chọn và cho zram biết cần có bao nhiêu thiết bị
được tạo trước. Mặc định: 1.

2) Chọn thuật toán nén
===============================
