...
:Original: Documentation/networking/device_drivers/ethernet/mellanox/mlx5/counters.rst
...
Các nhóm truy cập này dựa trên thành phần nào trong thiết lập mạng,
minh họa dưới đây, mà họ mô tả::
...
----------------------------------------
    ...
    ZZ0003ZZ ZZ0004ZZ |
    ZZ0005ZZ ZZ0006ZZ |
    Trình điều khiển Ethernet ZZ0007ZZ Trình điều khiển Ethernet ZZ0008ZZ RDMA Trình điều khiển ZZ0009ZZ ZZ0010ZZ Trình điều khiển Ethernet ZZ0011ZZ Trình điều khiển RDMA ZZ0012ZZ |
    ZZ0013ZZ ZZ0014ZZ |
    ZZ0015ZZ ZZ0016ZZ ZZ0017ZZ ZZ0018ZZ |
    ZZ0019ZZ ZZ0020ZZ |
    ZZ0021ZZ ZZ0022ZZ ZZ0023ZZ--
    ---------------------------------------- ----------------------------------------
                        ZZ0024ZZ
            ------------- --------------------
            ZZ0025ZZ
         ------ ------ ------ ------ ------ ------ ------
    -----ZZ0026ZZ----------------------ZZ0027ZZ-ZZ0028ZZ-ZZ0029ZZ----- --ZZ0030ZZ--- --ZZ0031ZZ--- --ZZ0032ZZ---
    ZZ0033ZZ ZZ0034ZZ ZZ0035ZZ ZZ0036ZZ
    ZZ0037ZZ ZZ0038ZZ ZZ0039ZZ ZZ0040ZZ
    ZZ0041ZZ ZZ0042ZZ ZZ0043ZZ ZZ0044ZZ
    ZZ0045ZZ ZZ0046ZZ ZZ0047ZZ ZZ0048ZZ
    ZZ0049ZZ ZZ0050ZZ ZZ0051ZZ ZZ0052ZZ
    ---------------------------------------------------------- ----------- ----------- ----------
               -------------------------------------------------------------------------------
               ZZ0053ZZ
               ZZ0054ZZ
...
Nhẫn
  Bộ đếm phần mềm được điền bởi ngăn xếp trình điều khiển.

Netdev
  Một tập hợp các bộ đếm vòng phần mềm.

bộ đếm vPort
  Bộ đếm giao thông và giảm do lái hoặc không có bộ đệm. Có thể chỉ ra các vấn đề
  với NIC. Các bộ đếm này bao gồm bộ đếm lưu lượng Ethernet (bao gồm cả Raw
  Ethernet) và bộ đếm lưu lượng RDMA/RoCE.

Bộ đếm cổng vật lý
  Bộ đếm thu thập số liệu thống kê về PF và VF. Có thể chỉ ra các vấn đề
  với NIC, liên kết hoặc mạng. Điểm đo này chứa thông tin về
  các bộ đếm được tiêu chuẩn hóa như IEEE 802.3, RFC2863, RFC 2819, RFC 3635 và
  các bộ đếm bổ sung như kiểm soát luồng, FEC và hơn thế nữa. Bộ đếm cổng vật lý
  không được tiếp xúc với máy ảo.
...
Quầy thông tin giao thông
  Bộ đếm đếm lưu lượng truy cập. Những bộ đếm này có thể được sử dụng để ước tính tải
  hoặc để gỡ lỗi chung.

Bộ đếm tăng tốc giao thông
  Bộ đếm đếm lưu lượng truy cập được tăng tốc bởi trình điều khiển Mellanox hoặc bởi
  phần cứng. Bộ đếm là một lớp bổ sung cho bộ đếm thông tin,
  và cùng một lưu lượng được tính trong cả bộ đếm thông tin và bộ đếm tăng tốc.

.. [#accel] Traffic acceleration counter.

Bộ đếm lỗi
  Việc tăng các bộ đếm này có thể cho thấy có vấn đề. Mỗi bộ đếm này
  có giải thích và hành động khắc phục.

Thống kê có thể được tìm nạp thông qua các lệnh ZZ0000ZZ hoặc ZZ0001ZZ. ZZ0002ZZ
cung cấp thông tin chi tiết hơn.::

hiển thị liên kết ip –s <if-name>
    ethtool -S <if-name>

Mô tả
============

Các bộ đếm XSK, PTP và QoS tương tự như các bộ đếm được xác định trước đó sẽ
không được liệt kê riêng biệt. Ví dụ: ZZ0000ZZ sẽ không
được ghi lại rõ ràng vì ZZ0001ZZ mô tả hành vi của cả hai
bộ đếm, ngoại trừ ZZ0002ZZ chỉ được tính khi thời gian chính xác
giao thức được sử dụng.

Bộ đếm vòng / Netdev
----------------------------
Các bộ đếm sau đây có sẵn trên mỗi vòng hoặc cổng phần mềm.

Các bộ đếm này cung cấp thông tin về lượng lưu lượng truy cập đã được tăng tốc
bởi NIC. Các bộ đếm đang đếm lưu lượng tăng tốc ngoài
bộ đếm tiêu chuẩn đếm nó (tức là lưu lượng truy cập tăng tốc được tính hai lần).

Tên bộ đếm trong bảng bên dưới đề cập đến cả bộ đếm vòng và bộ đếm cổng. các
ký hiệu cho bộ đếm vòng bao gồm chỉ số [i] không có dấu ngoặc nhọn. các
ký hiệu cho bộ đếm cổng không bao gồm [i]. Tên truy cập
ZZ0000ZZ sẽ được in dưới dạng ZZ0001ZZ cho vòng 0 và ZZ0002ZZ cho vòng 0
cổng phần mềm.
...
Bộ đếm vPort
--------------
Bộ đếm trên cổng NIC được kết nối với eSwitch.
...
Bộ đếm cổng vật lý
----------------------
Bộ đếm cổng vật lý là bộ đếm trên cổng bên ngoài kết nối với
bộ điều hợp vào mạng. Điểm đo này chứa thông tin về tiêu chuẩn hóa
các bộ đếm như IEEE 802.3, RFC2863, RFC 2819, RFC 3635 và các bộ đếm bổ sung
như kiểm soát luồng, FEC và hơn thế nữa.
...
Bộ đếm cổng ưu tiên
----------------------
Các bộ đếm sau đây là bộ đếm cổng vật lý được tính trên mỗi L2
mức độ ưu tiên (0-7).

ZZ0001ZZ ZZ0000ZZ trong tên bộ đếm thể hiện mức độ ưu tiên.
